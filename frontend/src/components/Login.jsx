...
const Login = () => {
  ...
  const handleGoogleSubmit = async (emailToUse) => {
    ...
    setLoading(true);
    try {
      await googleLogin(emailToUse || googleInputEmail, 'Google User', 'google_id_12345');
      setShowGooglePrompt(false);
    } catch (err) {
      setError(err.response?.data?.error || 'Google Sign-In failed. Account must be provisioned by HR.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-olive-50 p-4 font-sans">
      ...
      {showGooglePrompt && (
        <div className="fixed inset-0 bg-olive-950/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-sm bg-white border border-olive-200 rounded-2xl shadow-2xl p-6 space-y-4 text-center">
            <div className="w-12 h-12 rounded-full bg-olive-100 text-olive-700 flex items-center justify-center mx-auto">
            ...
            <h3 className="text-base font-bold text-olive-950">Google Sign-In Authentication</h3>
            <p className="text-xs text-olive-600">
              Select or enter your company Google email. Only pre-created employee accounts provisioned by HR will be authorized.
            </p>

            <div className="space-y-2 text-left">
              <button
                type="button"
                className="w-full text-left p-2.5 border border-olive-200 hover:bg-olive-50 rounded-xl text-xs font-semibold text-olive-900 flex items-center justify-between"
                onClick={() => handleGoogleSubmit('hr@company.com')}
              >
                <span>hr@company.com</span>
                <span className="text-[10px] bg-olive-900 text-white px-2 py-0.5 rounded-md">HR Admin</span>
              </button>
              <button
                type="button"
                className="w-full text-left p-2.5 border border-olive-200 hover:bg-olive-50 rounded-xl text-xs font-semibold text-olive-900 flex items-center justify-between"
                onClick={() => handleGoogleSubmit('mentor@company.com')}
              >
                <span>mentor@company.com</span>
                <span className="text-[10px] bg-olive-700 text-white px-2 py-0.5 rounded-md">Mentor</span>
              </button>
              <button
                type="button"
                className="w-full text-left p-2.5 border border-olive-200 hover:bg-olive-50 rounded-xl text-xs font-semibold text-olive-900 flex items-center justify-between"
                onClick={() => handleGoogleSubmit('mentee@company.com')}
              >
                <span>mentee@company.com</span>
                <span className="text-[10px] bg-olive-500 text-white px-2 py-0.5 rounded-md">Mentee</span>
              </button>
            </div>
            ...
            <div className="space-y-2">
              <input
                type="email"
                className="w-full px-3 py-2 bg-white border border-olive-200 rounded-xl text-xs text-olive-900 placeholder:text-olive-400"
                placeholder="Or type custom Google email..."
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                <button
                  type="button"
                  className="px-3 py-1.5 border border-olive-200 text-olive-800 text-xs font-semibold rounded-xl"
                ...
                <button
                  type="button"
                  className="px-4 py-1.5 bg-olive-700 text-white text-xs font-semibold rounded-xl shadow-md"
                  onClick={() => handleGoogleSubmit(googleInputEmail)}
                >
                  Authenticate
                </button>
              </div>
